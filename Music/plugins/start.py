from Music import Music, BOT_USERNAME
...
@Music.on(events.NewMessage(pattern="^[?!/]start$"))
async def start(event):
    ...
    if event.is_private:
       await event.client.send_file(event.chat_id,
        ...
        [Button.url("👨‍💻 ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ", "https://github.com/thorthunderbot/MusicBot")],
        [Button.url("🗣️ ꜱᴜᴘᴘᴏʀᴛ", f"https://t.me/{BOT.SUPPORT}"), Button.url("📣 ᴜᴘᴅᴀᴛᴇꜱ", f"https://t.me/{MyBotsupport}")],
...
@Music.on(events.callbackquery.CallbackQuery(data="start"))
async def _(event):
    if Config.MANAGEMENT_MODE == "ENABLE":
        return
    if event.is_private:
